...
class ModelArguments:
    ...
    # Hardy: I added this SER model configuration here.
    ser_model_name_or_path: str = field(
        default="firdhokk/speech-emotion-recognition-with-openai-whisper-large-v3",
        metadata={
            "help": "Used to compute emotion recognition accuracy during evaluation. Path to pretrained model or model identifier from huggingface.co/models"
        },
    )
...
class DataTrainingArguments:
    ...
    # Hardy: I added two attributes here
    emotion_column_name: str = field(
        default=None,
        metadata={"help": "The name of the dataset column containing the emotion labels. Defaults to 'None'."},
    )
    generated_audio_save_dir: str = field(
        default=None,
        metadata={"help": "Directory to save generated audio samples during training for post-training evaluation."},
    )
...
class ParlerTTSTrainingArguments(Seq2SeqTrainingArguments):
    ...
    # Hardy: I added two attributes here
    compute_ser_metric: bool = field(
        default=True,
        metadata={"help": ("Whether or not to compute the speech emotion recognition accuracy during evaluation.")},
    )
    post_training_generation_eval: bool = field(
        default=True,
        metadata={"help": "Whether to perform generation-based evaluation only after training is complete."},
    )